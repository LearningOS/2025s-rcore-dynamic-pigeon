...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let mut time_ptr = &time_val as *const TimeVal as *const u8;
    for buffer in buffers {
        buffer.copy_from_slice(unsafe { core::slice::from_raw_parts(time_ptr, buffer.len()) });
        time_ptr = unsafe { time_ptr.add(buffer.len()) };
    }
    0
}
