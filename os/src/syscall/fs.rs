...
use crate::fs::{link_count, linkat, open_file, unlinkat, OSInode, OpenFlags, Stat, StatMode};
...
pub fn sys_fstat(fd: usize, st: *mut Stat) -> isize {
    ...
    let inode = Arc::downcast::<OSInode>(inode).unwrap();

    let ino = inode.get_inode().inode_id() as u64;
...
pub fn sys_linkat(old_name: *const u8, new_name: *const u8) -> isize {
    ...
    if let Some(file) = open_file(&old_name, OpenFlags::RDONLY) {
        linkat(&new_name, &file.get_inode());
        0
    } else {
        -1
    }
...
pub fn sys_unlinkat(name: *const u8) -> isize {
    ...
    if let Some(_file) = open_file(&name, OpenFlags::RDONLY) {
        unlinkat(&name);
        0
    } else {
        -1
